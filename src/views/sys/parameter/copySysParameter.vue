    ...
    <el-form
      ...
      <div class="border-box">
          ...
          >
            <el-input
              ...
              autocomplete="off"
              placeholder="请输入副本参数名称"
            ></el-input>
          </el-form-item>
           <el-form-item  label-width="110px" label="副本参数描述" prop="v_Description" class='description-label'>
              <el-input
                v-model="modeAddData.v_Description"
                placeholder="请输入副本参数描述"
                type="textarea"
                class="minInput"
      ...
      <div class="border-box">
         <span>
          ...
          >
            <el-input
              v-model="modeAddData.v_ParamCode"
              disabled
              autocomplete="off"
              placeholder="请输入"
            ></el-input>
...
export default {
  ...
  created () {
    ...
    if (confsBtn.operationcode === 'copyData') {
      this.$set(this.modeAddData, 'v_ParamCode', formData.v_ParamCode)
      this.$set(this.modeAddData, 'v_Description', formData.v_Description)
      this.$set(this.modeAddData, 'v_CopyName', `${formData.v_ParamName}-副本`)
      this.componentName = formData.v_ParamName
    }
  ...
  methods: {
    ...
    commit () {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          cgiService.copySysParameter(this.modeAddData).then(res => {
            if (!res.code) {
