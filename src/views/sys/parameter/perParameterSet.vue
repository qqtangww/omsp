    ...
    commit () {
      this.$refs.modeAddData.validate(valid => {
        if (valid) {
          ...
          cgiService.addUserParameterGrp(this.modeAddData).then(res => {
            if (res.code === 0) {
              this.formRed = false
              if (res.iId > 1) {
              ...
              // 重新调取接口渲染数据
              let param = this.$store.state.paginations['CC_VW_parameterUser']
              this.$set(param, 'viewid', this.$store.state.fieRowTableData['CC_VW_parameterUser'].viewid)
