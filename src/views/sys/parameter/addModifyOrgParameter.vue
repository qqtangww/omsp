    ...
    >
      <div class="border-box">
        ...
        <div class="conBox">
          ...
          <el-select v-model="metaForm.v_OrgCode" multiple collapse-tags placeholder="请选择" :disabled="isDisable">
            <el-option
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        v_ParamValue: [
          { required: true, message: '请输入组织参数值1', trigger: 'blur' }
        ],
        v_OrgCode: [
  ...
  created () {
    ...
    if (confsBtn.operationcode === 'modify') {
      ...
      this.metaForm.v_OrgCode = [formData.v_OrgCode]
  ...
  methods: {
    commit () {
      ...
      this.$refs.metaForm.validate(valid => {
        if (valid) {
          ...
          cgiService.tempParameterSaveData3(params).then(res => {
            if (res.code === 0) {
              this.formRed = false
              if (res.iId > 1) {
              ...
              let params = this.$store.state.paginations['CC_VW_parameterOrg']
    ...
    getParameterSysList (id) {
      let params = { i_BizID: id, i_EnableOrg: '1' }
