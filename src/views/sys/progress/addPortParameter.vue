...
export default {
  ...
  methods: {
    commit () {
      let formData = this.$store.state.fieRowTableData['CC_VW_progress']
      let params = {
        ...this.metaForm
      }
      ...
      this.$set(params, 'v_Code', formData.v_ProcessCode)
      this.$refs.metaForm.validate(valid => {
        if (valid) {
          ...
          cgiService.progressSaveData6(params).then(res => {
            if (res.code === 0) {
              this.formRed = false
              if (res.iId > 1) {
              ...
              let params = this.$store.state.paginations['CC_VW_progressAPI']
    ...
    // 获取接口参数
    getParameterSysList () {
      cgiService.parameterApiList().then(res => {
        if (res.code === 0) {
