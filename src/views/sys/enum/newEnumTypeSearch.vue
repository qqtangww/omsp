   ...
   <div class="enum-type-custom-query">
    <el-form inline-message ref="form" class="form-search" :model="formSearch">
      <div class="input-container clearfix">
        <el-row>
          <el-col :span='15'>
             <el-form-item  label-width="80px" label="枚举类型">
                  <el-input v-model="formSearch.enumKeywords" placeholder="输入名称搜索" clearable @keydown.enter.native="onSubmit"></el-input>
              </el-form-item>
          </el-col>
...
export default {
  ...
  methods: {
    onSubmit () {
      let searchQuery = ''
      if (this.formSearch.enumKeywords !== '') {
        searchQuery = `(t_sys_enum.v_EnumName LIKE '%${this.formSearch.enumKeywords}%' OR t_sys_enum.v_EnumCode LIKE '%${this.formSearch.enumKeywords}%')`
      }
      let obj = {
      ...
      let row1 = this.$store.state.fieRowTableData.CC_CW_newEnumCategory
      if (row1.v_EnumCode) {
        this.$set(row, 'enumValue', true)
        row1.v_EnumCode = ''
