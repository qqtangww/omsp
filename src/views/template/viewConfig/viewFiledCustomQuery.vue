<!-- 视图字段自定义查询 -->
<template>
<div class='view-filed-custom-query'>
    ...
    onSubmit () {
      ...
      for (const key in this.formSearch) {
        ...
        if (this.formSearch[key]) {
          if (key === 'v_ColName') {
            params[key] = `t_sys_view_column.v_ColName LIKE '%${this.formSearch.v_ColName}%' `
          }
          if (key === 'i_ColTitle') {
            params[key] = `t_sys_view_column.i_ColTitle LIKE '%${this.formSearch.i_ColTitle}%'`
          }
          if (key === 'i_ColumnType') {
            params[key] = `t_sys_view_column.i_ColumnType LIKE '%${this.formSearch.i_ColumnType}%'`
          }
        }
      }
      // 拼接查询条件
...
.view-filed-custom-query /deep/.el-date-editor .el-range__icon {
    line-height: 24px;
}
.view-filed-custom-query /deep/ .el-form-item__content {
  width: 100%
}
.view-filed-custom-query /deep/ .el-form-item__content .el-range-editor{
  width: 100%
}
</style>
