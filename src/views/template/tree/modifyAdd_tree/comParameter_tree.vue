...
export default {
  ...
  methods: {
    commit () {
      ...
      this.$set(params, 'v_Code', formData.v_TreeCode)
        ...
        if (valid) {
          ...
          cgiService.getComParameter(params).then(res => {
            if (res.code === 0) {
              ...
              let params = this.$store.state.paginations['CC_VW_formCompParameter']
    ...
    // 获取组件参数
    getParameterSysList () {
      let params = { v_ComponentType: 'CC_TR' }
