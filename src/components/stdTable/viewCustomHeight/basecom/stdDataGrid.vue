 ...
 * @LastEditTime: 2019-09-26 15:21:45
 * @LastEditors: QWH
 -->
<template>
  ...
  methods: {
        ...
        this.$nextTick(() => {
          let tableFixedRightPatch =
            document.querySelector(
              `.table_${this.stdGridConf.viewid} .el-table__fixed-right-patch`
            ) || ''
    ...
    getData (interfaceData, val, rowData, scope) {
      ...
      if (interfaceData.columntype === '1') {
        ...
        return interfaceData.formatstring && interfaceData.dataformat ? global.formatData(interfaceData.formatstring, data, interfaceData.dataformat) : data
      }
    },
    // 处理switch函数
  ...
  mounted () {
    ...
    if (this.stdGridConf.isshowordercol === '1') {
      setTimeout(() => {
        ...
        const tbody = document.querySelector(`.table_${this.stdGridConf.viewid} .el-table__body-wrapper tbody`)
