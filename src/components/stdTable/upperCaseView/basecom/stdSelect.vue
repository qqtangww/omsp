  ...
  watch: {
    currentConfigData: {
      ...
      handler (nv) {
        ...
        // 是否缺省显示查询栏
        this.isenquirybarexpand = nv.confView.i_IsEnquiryBarExpand
        // 筛选查询字段
        nv.confColumn.map(val => {
          let obj = {}
          for (var key in val) {
            if (val.isselect === '1') {
              obj.searchtitle = val.i_ColTitle
              obj.colname = val.v_ColName
              obj.searchsubtype = val.i_SearchSubType
              this.isSelectArr.push(obj)
              return
            }
          }
        })
        if (this.isSelectArr.length < 1 || nv.confView.i_IsShowEnquiryBar === '0') {
  ...
  methods: {
    fieldSelect (item, index) {
      this.isSelectArr.forEach(val => {
        if (val.v_ColName === item.queryKey && val.i_SearchSubType === '2') {
