<template>
  <div :class="'selectComponent-' + configData.confView.viewid" class="selectComponent " v-if="isshowenquirybar === '1'" v-show="isenquirybarexpand === '1'">
    <!-- 条件下拉区域 -->
    <div class="selectDiv">
  ...
  methods: {
    ...
    searchtableData () {
      ...
      if (flagNull) {
        ...
        if (arr.length === 0) {
          var currentPage = this.$store.state.paginations[this.configData.confView.viewid]
          Object.assign(params, currentPage)
        }
        // getQueryData：把当前的查询条件传给父组件
